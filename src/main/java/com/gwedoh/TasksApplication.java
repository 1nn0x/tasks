...
public class TasksApplication {
    ...
    CommandLineRunner runner(TaskService taskService) {
        return args -> {
            ...
            taskService.save(new Task(3L, "Create the Task Domain Class", LocalDate.now().plus(3, ChronoUnit.MONTHS), false));
            taskService.save(new Task(4L, "Create service and repository classes", LocalDate.now().plus(5, ChronoUnit.DAYS), false));
            taskService.save(new Task(5L, "Create the command line runner to load data", LocalDate.now().plus(8, ChronoUnit.MONTHS), false));
            taskService.save(new Task(6L, "Create the required application properties", LocalDate.now().plus(10, ChronoUnit.DAYS), false));
